...
- Direct automation primitives remain best-effort and may use null/current-state/no-op results for unavailable targets or unusable inputs. Do not throw for ordinary user-level mistakes such as conflicting primitive arguments, invalid regexes, unknown option values, irrelevant fields, or invalid repeat counts; use null/no-match/no-op, ignore the irrelevant field, or apply the documented priority/fallback. Reserve exceptions for genuinely blocking backend/runtime failures where the native operation cannot proceed correctly. Inside `run()`, never abort the scenario for an ordinary step failure: convert it to a concise `{error, ...details}` result. Do not expose stacks. Preserve exception text only as a short `message` on `{error: "action failed", action, message}` when such a real backend failure occurs.
